...
module.exports = function(options) {
  ...
  function makeApiCall(req, res, next) {
    ...
    if (is.hasBody(req)) {
      ...
      // body-parser breaks piping request input so override if body is parsed
      if (typeof req.body === 'object' && typeof req.headers['content-type'] === 'string' && ~req.headers['content-type'].indexOf('application/x-www-form-urlencoded')) {
        apiRequestOptions.form = req.body;
